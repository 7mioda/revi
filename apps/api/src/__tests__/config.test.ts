...
describe('validateEnv', () => {
  it('returns parsed config when all required vars are present', () => {
    const result = validateEnv({ GITHUB_TOKEN: 'ghp_test', MONGODB_URI: 'mongodb://localhost/test', PORT: '4000' })
  ...
  it('uses PORT default of 3000 when not set', () => {
    const result = validateEnv({ GITHUB_TOKEN: 'ghp_test', MONGODB_URI: 'mongodb://localhost/test' })
